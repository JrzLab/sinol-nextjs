import { getClassByUidClassUser, getEventByUidClassUser, getTaskUserByUidAndEmail, getUsersClassByUidClass } from "@/app/actions/api-actions";
...
const TeacherEventPage = async ({ params }: { params: { classroomId: string; eventId: string } }) => {
  ...
  const getUserData = await getUsersClassByUidClass(classData?.uid!);
  const taskData = await getTaskUserByUidAndEmail(eventData?.id.toString()!, userEmailCookies!);
  return (
    <>
